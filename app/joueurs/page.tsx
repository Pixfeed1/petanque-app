...
import { Users, Plus, Search, Eye, Male, Female } from '@/components/Icons'
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-8">
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
          {[
            ...
            { label: 'Hommes', value: stats.hommes, icon: <Male className="w-10 h-10 sm:w-12 sm:h-12 text-blue-500" /> },
            { label: 'Femmes', value: stats.femmes, icon: <Female className="w-10 h-10 sm:w-12 sm:h-12 text-pink-500" /> }
          ].map((stat, index) => (
            <div
              key={index}
                ...
                <div className="relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all group-hover:-translate-y-1">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1">
                      {/* Avatar avec icône de genre */}
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                        joueur.gender === 'F'
                          ? 'bg-pink-100 text-pink-600'
                          : 'bg-blue-100 text-blue-600'
                      }`}>
                        {joueur.gender === 'F' ? <Female className="w-7 h-7" /> : <Male className="w-7 h-7" />}
                      </div>
